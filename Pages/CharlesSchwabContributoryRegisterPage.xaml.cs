...
{
    public partial class CharlesSchwabContributoryRegisterPage : ContentPage
    {
        private readonly string registerFile = FilePathHelper.GetKukiFinancePath("CharlesSchwabContributory.csv");
        private readonly string currentFile = FilePathHelper.GetKukiFinancePath("CharlesSchwabContributoryCurrent.csv");
        private readonly string categoryFile = FilePathHelper.GetKukiFinancePath("Category.csv");
        private readonly decimal openingBalance = OpeningBalances.Get("CharlesSchwabContributory"); // Set your actual opening balance

        private readonly RegisterViewModel viewModel = new();

        public CharlesSchwabContributoryRegisterPage()
        {
            InitializeComponent();
        ...
        private void LoadRegister()
        {
            ...
            var entries = RegisterService.LoadRegister<RegistryEntry>(
                ...
                new RegistryEntry
                {
                    Date = OpeningBalances.GetDate("CharlesSchwabContributory") ?? DateTime.Today.AddDays(-1),
        ...
        private async void ManualTransactionEntryButton_Clicked(object sender, EventArgs e)
        {
            ...
            string balanceStr = await DisplayPromptAsync("Manual Entry", "Enter new balance amount:");
            if (string.IsNullOrWhiteSpace(balanceStr) || !decimal.TryParse(balanceStr, out var newBalance))
            {
                await DisplayAlert("Invalid", "Please enter a valid balance amount.", "OK");
                return;
            }

            // Get the previous balance (last entry)
            decimal previousBalance = 0m;
            if (viewModel.Entries.Count > 0)
                previousBalance = viewModel.Entries.Last().Balance;

            // Calculate the new amount
            decimal amount = newBalance - previousBalance;
            ...
            await DisplayAlert("Success", $"Manual transaction added. Amount calculated: {amount:C}", "OK");
        }
        private async void EditButton_Clicked(object sender, EventArgs e)
        {
            if (RegisterCollectionView.SelectedItem is RegistryEntry selectedEntry)
            {
                ...
                // Save the change to CharlesSchwabContributory.csv
