...
{
    public partial class HealthProRegisterPage : ContentPage
    {
        private readonly string registerFile = FilePathHelper.GetKukiFinancePath("HealthPro.csv");
        private readonly string currentFile = FilePathHelper.GetKukiFinancePath("HealthProCurrent.csv");
        private readonly string categoryFile = FilePathHelper.GetKukiFinancePath("Category.csv");
        private readonly decimal openingBalance = OpeningBalances.Get("HealthPro");

        private readonly RegisterViewModel viewModel = new();

        public HealthProRegisterPage()
        {
            InitializeComponent();
        ...
        private void LoadRegister()
        {
            ...
            var entries = RegisterService.LoadRegister<RegistryEntry>(
                ...
                new RegistryEntry
                {
                    Date = OpeningBalances.GetDate("HealthPro") ?? DateTime.Today.AddDays(-1),
        ...
        private async void EditButton_Clicked(object sender, EventArgs e)
        {
            if (RegisterCollectionView.SelectedItem is RegistryEntry selectedEntry)
            {
                ...
                // Save the change to HealthPro.csv
