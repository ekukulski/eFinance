...
{
    public partial class HouseRegisterPage : ContentPage
    {
        private readonly string registerFile = FilePathHelper.GetKukiFinancePath("House.csv");
        private readonly string currentFile = FilePathHelper.GetKukiFinancePath("HouseCurrent.csv");
        private readonly string categoryFile = FilePathHelper.GetKukiFinancePath("Category.csv");
        private readonly decimal openingBalance = OpeningBalances.Get("House");

        private readonly RegisterViewModel viewModel = new();

        public HouseRegisterPage()
        {
            InitializeComponent();
        ...
        private void LoadRegister()
        {
            ...
            var entries = RegisterService.LoadRegister<RegistryEntry>(
                ...
                new RegistryEntry
                {
                    Date = OpeningBalances.GetDate("House") ?? DateTime.Today.AddDays(-1),
        ...
        private async void ManualTransactionEntryButton_Clicked(object sender, EventArgs e)
        {
            ...
            // Prompt for Realtor Price
            string realtorStr = await DisplayPromptAsync("Manual Entry", "Enter Realtor price:");
            if (string.IsNullOrWhiteSpace(realtorStr) || !decimal.TryParse(realtorStr, out var realtorPrice))
            {
                await DisplayAlert("Invalid", "Please enter a valid Realtor price.", "OK");
                return;
            }

            // Prompt for Redfin Price
            string redfinStr = await DisplayPromptAsync("Manual Entry", "Enter Redfin price:");
            if (string.IsNullOrWhiteSpace(redfinStr) || !decimal.TryParse(redfinStr, out var redfinPrice))
            {
                await DisplayAlert("Invalid", "Please enter a valid Redfin price.", "OK");
                return;
            }

            // Prompt for Zillow Price
            string zillowStr = await DisplayPromptAsync("Manual Entry", "Enter Zillow price:");
            if (string.IsNullOrWhiteSpace(zillowStr) || !decimal.TryParse(zillowStr, out var zillowPrice))
            {
                await DisplayAlert("Invalid", "Please enter a valid Zillow price.", "OK");
                return;
            }

            // Calculate average price and amount
            decimal averagePrice = (realtorPrice + redfinPrice + zillowPrice) / 3m;
            decimal previousBalance = viewModel.Entries.Count > 0 ? viewModel.Entries.Last().Balance : 0m;
            decimal amount = averagePrice - previousBalance;

            // Compose the CSV line
            string csvLine = $"{date:MM/dd/yyyy},{description},{amount}";
            ...
            await DisplayAlert("Success", $"Manual transaction added. Average price: {averagePrice:C}, Amount: {amount:C}", "OK");
        }
