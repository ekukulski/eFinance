...
{
    public partial class NissanSentraRegisterPage : ContentPage
    {
        private readonly string registerFile = FilePathHelper.GetKukiFinancePath("NissanSentra.csv");
        private readonly string currentFile = FilePathHelper.GetKukiFinancePath("NissanSentraCurrent.csv");
        private readonly string categoryFile = FilePathHelper.GetKukiFinancePath("Category.csv");
        private readonly decimal openingBalance = OpeningBalances.Get("NissanSentra");

        private readonly RegisterViewModel viewModel = new();

        public NissanSentraRegisterPage()
        {
            InitializeComponent();
        ...
        private void LoadRegister()
        {
            ...
            var entries = RegisterService.LoadRegister<RegistryEntry>(
                ...
                new RegistryEntry
                {
                    Date = OpeningBalances.GetDate("NissanSentra") ?? DateTime.Today.AddDays(-1),
        ...
        private async void EditButton_Clicked(object sender, EventArgs e)
        {
            if (RegisterCollectionView.SelectedItem is RegistryEntry selectedEntry)
            {
                ...
                // Save the change to NissanSentra.csv
